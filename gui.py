    ...
    def desenhaGrafo(self, dg = DiGraph.DiGraph()):
        ...
        class Circle():
            ...
            def dist(self, x, y):
                ...
                return dist
        while (len(self.lst) < dg.numVertices()):
            circle = Circle()
            overlapping = False
        ...
        for value in dg.vertices():
            for edge in dg.adjacentTo(value):
                ...
                #if valueTag=='Pie2':
                self.canvas1.create_line(x1,y1, x2, y2, tag=valueTag) 
                #self.coordPolar(x1, y1)
                #self.canvas1.create_line(self.coordPolar(x1, y1)[0], self.coordPolar(x1, y1)[1],
                #x2, y2)
                    #
                #else:
                ...
                #print('value', value, 'edge', edge, 'dg.adjacentTo(value)', dg.adjacentTo(value))
    '''def coordPolar(self, x1, y1):
        print(y1/x1)
        teta = math.atan(y1/x1)
        p1 = 2*30*math.cos(teta + math.pi/2)*x1
        p2 = 2*30*math.sin(teta + math.pi/2)*y1
        r = (p1 + p2
         - (x1**2 + y1**2))
        x =  r * math.sin(teta - math.pi)
        y = r * math.cos(teta - math.pi)
        return x, y'''
        ...
        for v in  self.canvas1.find_overlapping(inte[0]+1, inte[1]+1, inte[2]+1, inte[3]+1):
            if edge in self.canvas1.gettags(v):
                ...
                for v2 in self.canvas1.find_overlapping(inte2[0]+1, inte2[1]+1, inte2[2]+1, inte2[3]+1):
                    if edge in self.canvas1.gettags(v):
                        self.canvas1.create_line(inte2[0],inte2[1], inte[0], inte[1], tag=edge) 
                        overlapping = True
                        break
                self.canvas1.create_line(x, y,  inte[0], inte[1], tag=edge)
        if not overlapping:
             for v2 in self.canvas1.find_overlapping(inte2[0]+1, inte2[1]+1, inte2[2]+1, inte2[3]+1):
                 if edge in self.canvas1.gettags(v2):
                        self.canvas1.delete(edge)
                        self.canvas1.create_line(inte2[0],inte2[1], inte[2], inte[3], tag=edge)
                        break  
